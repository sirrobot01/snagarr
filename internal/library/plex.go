...
func NewPlex(baseURL, token string) *Plex {
	return &Plex{rest: httpx.Client{BaseURL: baseURL, Header: http.Header{"X-Plex-Token": {token}}}}
}
...
// show section.
func (p *Plex) Items(ctx context.Context, sectionIDs []string, since time.Time) ([]Item, error) {
	if len(sectionIDs) == 0 {
		sections, err := p.Sections(ctx)
		if err != nil {
			return nil, err
...
func (m plexMetadata) item(t media.Type) Item {
	it := Item{ProviderItemID: m.RatingKey, Type: t, Title: m.Title, Year: m.Year}
	if m.AddedAt > 0 {
...
func plexType(kind string) (media.Type, bool) {
	switch kind {
	case "movie":
