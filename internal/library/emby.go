...
func NewEmby(baseURL, apiKey string, jellyfin bool) *Emby {
	return &Emby{
		rest:     httpx.Client{BaseURL: baseURL, Header: http.Header{"X-Emby-Token": {apiKey}}},
		jellyfin: jellyfin,
	}
}
...
func (e *Emby) Items(ctx context.Context, sectionIDs []string, since time.Time) ([]Item, error) {
	parents := sectionIDs
	if len(parents) == 0 {
		parents = []string{""}
	}
...
func (i embyItem) item() (Item, bool) {
	var t media.Type
	switch i.Type {
	case "Movie":
		t = media.Movie
