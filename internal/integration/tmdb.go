...
// TMDBClient calls the TMDB v3 API for search, resolution and metadata.
...
// TMDBExternalIDs holds the non-TMDB identifiers used to match *arr and library
// items.
...
func (c *TMDBClient) SearchMulti(ctx context.Context, query string) ([]Result, error) {
	var body struct {
...
func (c *TMDBClient) Details(ctx context.Context, t store.MediaType, id int) (*Details, error) {
	if !t.Valid() {
...
func (c *TMDBClient) ExternalIDs(ctx context.Context, t store.MediaType, id int) (TMDBExternalIDs, error) {
	if !t.Valid() {
		return TMDBExternalIDs{}, fmt.Errorf("tmdb external ids: unknown media type %q", t)
	}
	var body struct {
	...
	if err := c.get(ctx, path, nil, detailsTTL, &body); err != nil {
		return TMDBExternalIDs{}, fmt.Errorf("tmdb external ids %s/%d: %w", t, id, err)
	}
	return TMDBExternalIDs{IMDBID: body.IMDBID, TVDBID: body.TVDBID}, nil
}

// FindByIMDB resolves an IMDB id to a movie or TV result. It returns ErrNotFound
// when TMDB knows no title with that id.
func (c *TMDBClient) FindByIMDB(ctx context.Context, imdbID string) (*Result, error) {
	var body struct {
	...
	if len(body.Movies) > 0 {
		r := body.Movies[0].result(store.Movie)
		return &r, nil
	}
	if len(body.TV) > 0 {
		r := body.TV[0].result(store.TV)
		return &r, nil
	}
...
func (c *TMDBClient) Ping(ctx context.Context) (string, error) {
...
func (c *TMDBClient) get(ctx context.Context, path string, query url.Values, ttl time.Duration, out any) error {
	key := path
	if len(query) > 0 {
		key += "?" + query.Encode()
	}
...
func (e entity) result(t store.MediaType) Result {
	r := Result{
		TMDBID:     e.ID,
		Type:       t,
		Title:      e.Title,
...
func typeOf(mediaType string) (store.MediaType, bool) {
	switch mediaType {
	case "movie":
