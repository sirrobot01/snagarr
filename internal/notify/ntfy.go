// Package notify publishes Snagarr's availability pushes.
package notify
...
func NewNtfy(serverURL, topic, token string) *Ntfy {
	...
	return &Ntfy{
		rest:  httpx.Client{BaseURL: serverURL, HTTP: &http.Client{Timeout: 20 * time.Second}, Header: header},
		topic: topic,
	}
}
...
func (n *Ntfy) Send(ctx context.Context, m Message) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		...
		return fmt.Errorf("ntfy send: %w", &httpx.Error{Status: resp.StatusCode, Body: string(bytes.TrimSpace(b))})
	}
	return nil
}

// Ping checks that the ntfy server is reachable and healthy.
func (n *Ntfy) Ping(ctx context.Context) error {
	var body struct {
