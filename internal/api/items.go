...
func (s *Server) resolveItem(w http.ResponseWriter, r *http.Request) {
	...
	s.sender.AutoSend(r.Context(), it.ID)
...
func (s *Server) sendItem(w http.ResponseWriter, r *http.Request) {
	...
	owners := s.sender.Owners(ctx, userFrom(r), it.CapturedBy)

	status, err := s.sender.Send(ctx, house, *it, req.Target, owners)
