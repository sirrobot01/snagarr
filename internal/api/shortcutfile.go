...
// The generator below builds a ready-to-import Apple Shortcut, so an operator
// can send a household member a link instead of dictating the steps to build
// one by hand.
//
...
// shortcutOptions describes the Shortcut to generate.
...
// defaultShortcutName is used when shortcutOptions.Name is empty.
const defaultShortcutName = "Snag"
...
// buildShortcutFile returns an XML plist ready to serve as a .shortcut file.
func buildShortcutFile(opts shortcutOptions) ([]byte, error) {
...
func writePlist(b *bytes.Buffer, v any) {
	switch v := v.(type) {
	case string:
