...
import (
	...
	"github.com/sirrobot01/snagarr/internal/capture"
...
func (r *Resolver) Resolve(ctx context.Context, client *tmdb.Client, itemID int64) error {
	...
	query, ref := it.RawInput, capture.Ref{}
	if capture.IsURL(it.RawInput) {
...
func (r *Resolver) dereference(ctx context.Context, raw string) capture.Ref {
	if ref, ok := capture.ParseURL(raw); ok {
		return ref
	}
	ref, err := capture.Scrape(ctx, r.http, raw)
	if err != nil {
...
func (r *Resolver) resolveByID(ctx context.Context, client *tmdb.Client, it *store.Item, tmdbID int64, t media.Type) error {
	if !t.Valid() {
		t = media.Movie
	}
...
func (r *Resolver) commit(ctx context.Context, client *tmdb.Client, it *store.Item, c store.Candidate) error {
...
func (r *Resolver) cacheEntity(ctx context.Context, client *tmdb.Client, tmdbID int64, t media.Type) {
	details, err := client.Details(ctx, t, int(tmdbID))
	if err != nil {
...
// few, best first.
func rank(query string, results []tmdb.Result) []store.Candidate {
