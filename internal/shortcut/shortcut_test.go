...
func parse(t *testing.T, opts Options) node {
	t.Helper()
	out, err := Build(opts)
	if err != nil {
	...
	if err := xml.Unmarshal(out, &plist); err != nil {
		t.Fatalf("Build output is not well-formed XML: %v", err)
	}
...
func TestBuildIsAWellFormedPlistWithTheExpectedActions(t *testing.T) {
	root := parse(t, Options{BaseURL: "https://snagarr.example.com", Token: "abc123"})
...
func TestBuildEscapesMarkupInTheToken(t *testing.T) {
	...
	out, err := Build(Options{BaseURL: "https://snagarr.example.com/?a=1&b=2", Token: nasty})
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
	...
	root := parse(t, Options{BaseURL: "https://snagarr.example.com/?a=1&b=2", Token: nasty})
	params := at(t, root, "WFWorkflowActions", 0, "WFWorkflowActionParameters")
...
func TestBuildDefaultsNameAndTrimsTrailingSlash(t *testing.T) {
	...
	if got := at(t, root, "WFWorkflowName").Chardata; got != DefaultName {
		t.Errorf("WFWorkflowName = %q, want %q", got, DefaultName)
	}
	...
	named := parse(t, Options{BaseURL: "https://snagarr.example.com", Token: "abc123", Name: "Snag it"})
