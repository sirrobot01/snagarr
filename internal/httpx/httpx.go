// Package httpx is the JSON-over-HTTP transport shared by every Snagarr
// integration client, so none of them reimplement request plumbing.
package httpx
...
// maxErrorBody caps how much of a failing response body an *Error keeps.
const maxErrorBody = 512

var defaultHTTP = &http.Client{Timeout: 20 * time.Second}

// Client talks JSON to a service rooted at BaseURL.
type Client struct {
	BaseURL string
...
// Error is returned for every response outside the 2xx range.
type Error struct {
	Status int
	Body   string
}
...
// StatusOf returns the HTTP status carried by err, or 0 when err is not an *Error.
...
func (c *Client) do(ctx context.Context, method, path string, query url.Values, body, out any) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		...
		return &Error{Status: resp.StatusCode, Body: string(bytes.TrimSpace(b))}
	}
